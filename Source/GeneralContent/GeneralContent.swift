...
public class GeneralContent: NSObject {
    ...
    */
    public func getInstances(moduleIds moduleIds: [String], offlinePolicy: OfflinePolicy = .LoadAndStoreLocalData,
        completionHandler handler: (Alamofire.Result<[Halo.GeneralContentInstance], NSError>, Bool) -> Void) -> Void {
            
            switch offlinePolicy {
            case .None:
                net.generalContentInstances(moduleIds, flags: [], completionHandler: handler)
            case .LoadAndStoreLocalData, .ReturnLocalDataDontLoad:
                persistence.generalContentInstances(moduleIds, flags: [], fetchFromNetwork: (offlinePolicy == .LoadAndStoreLocalData), completionHandler: handler)
            }
    }
